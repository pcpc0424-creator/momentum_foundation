...
const Notice = () => {
  ...
  const getDefaultNotices = (): NoticePost[] => [
    {
      id: 1,
      title: t('news.notice.empty.title'),
      content: t('news.notice.empty.content'),
      category: t('news.notice.empty.category'),
      type: "notice",
  ...
  useEffect(() => {
    ...
    const savedPosts = localStorage.getItem("notice_posts");

    if (savedPosts) {
      try {
        const posts = JSON.parse(savedPosts);
        if (posts.length > 0) {
          setNotices(posts.sort((a: NoticePost, b: NoticePost) => new Date(b.date).getTime() - new Date(a.date).getTime()));
        } else {
    ...
    const selectedPostId = localStorage.getItem("selectedNoticeId");
    if (selectedPostId && savedPosts) {
      try {
        const posts = JSON.parse(savedPosts);
        const selectedPost = posts.find((post: NoticePost) => post.id.toString() === selectedPostId);
        if (selectedPost) {
          const updatedPosts = posts.map((post: NoticePost) =>
            post.id === selectedPost.id ? { ...post, views: post.views + 1 } : post
          );
          localStorage.setItem("notice_posts", JSON.stringify(updatedPosts));
          setNotices(updatedPosts.sort((a: NoticePost, b: NoticePost) => new Date(b.date).getTime() - new Date(a.date).getTime()));
          setSelectedNotice({ ...selectedPost, views: selectedPost.views + 1 });
          localStorage.removeItem("selectedNoticeId");
        }
      } catch (error) {
  ...
  const handleNoticeClick = (notice: NoticePost) => {
    ...
    localStorage.setItem("notice_posts", JSON.stringify(updatedNotices));
                ...
                {selectedNotice.attachmentFile && (
                  <div className="mt-12 pt-8 border-t border-gray-100">
                    ...
                    <a
                      ...
                      download={selectedNotice.attachmentFileName || t('news.attachment')}
                    ...
                    >
                      ...
                      <span className="text-gray-700 font-medium">{selectedNotice.attachmentFileName || t('news.attachment')}</span>
                    </a>
                  </div>
                )}
              </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
          <div className={`text-center transform transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-white/5 border border-white/10 backdrop-blur-xl mb-10">
              ...
              <span className="text-emerald-300 text-sm font-medium tracking-wider uppercase">Notice Board</span>
            </div>
